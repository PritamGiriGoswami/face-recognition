  ...
  static Future<List<double>?> getEmbeddings(File imageFile) async {
    ...
    try {
      ...
      final faces = await _detector.processImage(inputImage).timeout(
        ...
        onTimeout: () => throw Exception('Face detection timed out.'),
      );
      ...
      // We use a flat Float32List which is 100x faster to populate than nested lists.
      ...
      for (int py = 0; py < 112; py++) {
        for (int px = 0; px < 112; px++) {
          ...
          input[outIndex++] = (r - 127.5) / 128.0; 
          input[outIndex++] = (g - 127.5) / 128.0; 
          input[outIndex++] = (b - 127.5) / 128.0; 
      ...
      // 6. Run Inference
      // Reshape the flat Float32List into the nested shape expected by TFLite
